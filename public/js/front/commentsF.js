/*eslint-disable*/
import { postComment, updateComment, deleteComment } from '../back/comments.js';
...
document.addEventListener('DOMContentLoaded', () => {
  // Handle delete buttonsdocument.addEventListener('DOMContentLoaded', () => {
  const urlParts = window.location.pathname.split('/');
  const username = urlParts[1];
  const slug = urlParts[2];
  ...
  document.querySelectorAll('.comment-reply-button').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (replyForm) {
        ...
        document.getElementById(`comment-editor-${commentId}`).focus();
      }
    });
  });
  ...
  document.querySelectorAll('.reply-form').forEach((form) => {
    form.addEventListener('submit', async (e) => {
      ...
      const elements = [];
      elements.push({ type: 'text', value: 'Setup gathering elements' });
      await postComment(elements, username, slug, commentId);
    });
  });
  ...
  document.querySelectorAll('.comment-edit-button').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (editForm) {
        ...
        const commentEditor = editForm.querySelector('comment-editor');
        commentEditor.focus();
      }
    });
  });
  ...
  document.addEventListener('click', (e) => {
    ...
    if (commentEditor) {
      e.preventDefault();
      console.log('Clicked comment editor:', commentEditor.id);
